...
export function getCenter(coords: RectangleType | PolygonType, isBox: boolean): CoordsType | null {
  ...
  if (isBox) {
    ...
    const lat = (granulaCoords.reduce((acc, currVal) => acc + currVal.lat, 0)) / granulaCoords.length;
    const lng = (granulaCoords.reduce((acc, currVal) => acc + currVal.lng, 0)) / granulaCoords.length;
